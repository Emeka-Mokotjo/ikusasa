...
function GraduateApplicationsPage() {
  const user = useAuthStore((s) => s.user);
  const userId = user?.id ?? "grad_1";
  ...
  return (
    <AppShell title="My applications" description="Every role you've applied to, in one place.">
      {isLoading ? (
            ...
            >
              <div className="grid gap-3 sm:grid-cols-[1fr_auto] sm:items-center">
                ...
                <div className="flex flex-col items-start gap-2 sm:items-end">
                  ...
                  <p className="text-xs text-muted-foreground">Sent {formatDate(a.submittedAt)}</p>
                </div>
              </div>
            </article>
          ))}
